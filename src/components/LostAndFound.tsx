      ...
      <div className="bg-navy-800 border border-slate-700/60 rounded-2xl p-6 shadow-xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-700/60 pb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-offwhite">Lost & Found</h2>
              <p className="text-xs text-slate-400">Report or find missing items on campus.</p>
            </div>
          </div>
          <div className="flex bg-navy-900 rounded-xl p-1 border border-slate-700">
            <button
              onClick={() => setActiveTab('browse')}
              className={`px-4 py-2 rounded-lg text-xs font-bold transition-colors ${
                activeTab === 'browse' ? 'bg-amber-500 text-navy-900 shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              Browse Items
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-bold transition-colors ${
                activeTab === 'report' ? 'bg-amber-500 text-navy-900 shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              Report Item
            </button>
        ...
        {activeTab === 'browse' ? (
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
              {isLoading ? (
                <div className="text-xs text-slate-400 col-span-2">Loading items...</div>
              ) : items.length === 0 ? (
                <div className="text-xs text-slate-400 text-center py-8 col-span-2">No lost or found items reported yet.</div>
              ) : (
                items.map((item) => (
                  <div key={item.id} className="bg-navy-900/80 p-4 rounded-xl border border-slate-700/50 space-y-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <h4 className="text-sm font-bold text-offwhite">{item.itemName}</h4>
          ...
          <div className="max-w-xl mx-auto space-y-4">
            <h3 className="text-sm font-bold text-offwhite">File a Report</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-navy-900 border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-offwhite focus:outline-none focus:border-amber-500"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-navy-900 border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-offwhite focus:outline-none focus:border-amber-500 resize-none"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1.5">Location (Optional)</label>
                <input
                  type="text"
                  value={location}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1.5">Contact Info (Optional)</label>
                <input
                  type="text"
                  value={contactInfo}
