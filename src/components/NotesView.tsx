...
export const NotesView: React.FC<NotesViewProps> = ({ user }) => {
  ...
  return (
    <div className="max-w-4xl w-full mx-auto space-y-6 pb-20 md:pb-6">
      <div className="bg-navy-800 border border-slate-700/60 rounded-2xl p-6 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Access lecture summaries, revision packs, and collaborative course materials.</p>
          </div>
        </div>
      ...
      {isLoading ? (
        <div className="text-xs text-slate-400 text-center py-8">Loading notes...</div>
      ) : filteredNotes.length === 0 ? (
        <div className="text-xs text-slate-400 text-center py-8">No notes found.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredNotes.map((note) => (
            <div key={note.id} className="bg-navy-800 border border-slate-700/60 rounded-2xl p-5 shadow-md flex flex-col justify-between space-y-4 hover:border-slate-600 transition-all group">
              <div className="space-y-2">
                ...
                <h3 className="text-base font-bold text-offwhite group-hover:text-emerald-300 transition-colors line-clamp-2">
                  {note.title}
                </h3>
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-navy-800 border border-slate-700 rounded-2xl p-6 max-w-lg w-full shadow-2xl space-y-4 animate-in fade-in duration-200">
            <div className="flex items-center justify-between border-b border-slate-700/60 pb-3">
              <h3 className="text-lg font-bold text-offwhite">Upload Course Notes</h3>
              <button
            ...
            <form onSubmit={handleCreateNote} className="space-y-4 text-xs">
              <div className="space-y-1">
                ...
                <input type="text" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="e.g. Marketing Research Methods Summary" className="w-full bg-navy-900 border border-slate-700 rounded-xl px-3 py-2 text-offwhite focus:outline-none focus:border-emerald-500" required />
              </div>
              <div className="space-y-1">
                <label className="block text-slate-300 font-medium">Course Code</label>
                <input type="text" value={newCourseCode} onChange={(e) => setNewCourseCode(e.target.value)} placeholder="MKT 302" className="w-full bg-navy-900 border border-slate-700 rounded-xl px-3 py-2 text-offwhite font-mono focus:outline-none focus:border-emerald-500" required />
              </div>
              <div className="space-y-1">
                <label className="block text-slate-300 font-medium">Document File</label>
                <input type="file" onChange={(e) => setSelectedFile(e.target.files?.[0] || null)} className="w-full bg-navy-900 border border-slate-700 rounded-xl px-3 py-2 text-offwhite focus:outline-none focus:border-emerald-500" required />
              </div>
              <div className="flex justify-end space-x-2 pt-2">
                <button type="button" onClick={() => setIsCreating(false)} className="px-4 py-2 bg-navy-900 border border-slate-700 text-slate-300 font-semibold rounded-xl">Cancel</button>
